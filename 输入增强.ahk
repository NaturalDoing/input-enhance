...
是中文状态(设置为中文 := false) {
    ...
    if (设置为中文 && 输入法状态 == 0) {
        SendMessage(0x283, 0x006, 1, , "ahk_id" 活动窗口的输入法窗口句柄id) ; 发送 WM_IME_CONTROL 消息，wParam=0x006(IMM_SET_OPEN_STATUS)，lParam=1 打开中文输入法
    }
...
中文状态双击输出英文符号(输入按键, 删除次数, 是上档键, *) {
    ...
    if (当前时间 - 第一次按键时间 < 双击间隔 && 输入按键 == 第一次按键符号) {
        ...
        if (上次双击按键符号 == 第一次按键符号 && 是固定时间内三击 && 是中文状态()) {
            if (第一次按键符号 == "]") {
                中文状态双击输出英文符号_有成对符号的左边键_三击(第一次按键符号, 0, 1)
            }
        }
...
中文状态双击输出英文符号_有成对符号的左边键(右移次数_双击, 删除次数_双击, 右移次数_三击, 删除次数_三击, 左下档键, 右下档键, 左上档键, 右上档键) {
    global 第一次按键时间
    global 第一次按键符号
    global 上次双击按键时间
    global 上次双击按键符号
    ...
    if (当前时间 - 第一次按键时间 < 双击间隔 && (左下档键 == 第一次按键符号 || 左上档键 == 第一次按键符号)) {
        ...
        if (上次双击按键符号 == 第一次按键符号 && 是固定时间内三击 && 是中文状态()) {
            if (第一次按键符号 == "``" || 第一次按键符号 == "[") {
                中文状态双击输出英文符号_有成对符号的左边键_三击(第一次按键符号, 右移次数_三击, 删除次数_三击)
            }
        }
...
中文状态双击输出英文符号_有成对符号的左边键_三击(输入符号, 右移次数_三击, 删除次数_三击) {
    if (输入符号 == "``") {
        ; `键三击输出代码块格式的符号(```...```)
        Send("{Right " 右移次数_三击 "}")
...
; 成对符号左下档键
~9:: {
    中文状态双击输出英文符号_有成对符号的左边键("1", "3", "1", "2", "9", "0", "(", ")")
}
~[:: {
    中文状态双击输出英文符号_有成对符号的左边键("2", "4", "1", "2", "[", "]", "{", "}")
}
~':: {
    中文状态双击输出英文符号_有成对符号的左边键("2", "4", "1", "2", "'", "'", '"', '"')
}
~,:: {
    中文状态双击输出英文符号_有成对符号的左边键("0", "2", "1", "2", ",", ".", "<", ">")
}
~`:: {
    中文状态双击输出英文符号_有成对符号的左边键("0", "2", "0", "1", "``", "``", '``', '``')
}
